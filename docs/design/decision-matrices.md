...
| Root observation policy | Expose copied Kafka policy events through bounded synchronous callbacks. | Producer delivery, consumer processing/commit/group lifecycle, producer and consume-transform-produce transaction lifecycle, producer/consumer/transaction-processor/replay/inspector shutdown lifecycle, replay plan/record/run lifecycle, inspector read/health/readiness lifecycle, and every client role's broker events contain no payloads, credentials, endpoints, arbitrary headers, or franz-go types. | Delivery, nontransactional consumption, group, transaction, replay, inspector, shutdown, and broker activity implemented |
| Test support | Publish conformance suites only for real consumer-facing seams. | No exported franz-go fakes or mock choreography contracts. | Producer, consumer, transaction, replay, inspector, authentication-provider, and observer suites implemented |
| Application adapters | CloudEvents, outbox, event-sourcing, service health, schemas, and routing stay downstream. | The root never imports application or another golib module; the generated manifest defines the complete reverse-dependency set. | Dependency direction implemented; final fingerprint-scoped consumer evidence pending |
...
| Topic routing | Require a bounded immutable producer topic allowlist. | A zero-value configuration cannot produce to arbitrary topics; denied records fail before franz-go admission. |
| Protocol versions | Negotiate request versions per broker connection; optionally set a validated minimum request-version floor. | The package does not expose franz-go version types, pin a maximum, infer broker release, or turn upstream protocol support into a support claim. |
