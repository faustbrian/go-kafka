...
```go
instrumentation, err := kafkaotel.New(kafkaotel.Config{
    Runtime: telemetryRuntime,
    Attributes: kafkaotel.AttributePolicy{
...
```go
tracePolicy, err := kafkaotel.NewTraceContextPropagation(messageLimits)
if err != nil {
    return err
}
...
Observable interpretation choices remain covered by the
[compatibility contract](../COMPATIBILITY.md), contract tests, and the reviewed
source history retained by the predecessor module.

```sh
make check
```

The canonical-path module gate covers formatting, vet, unit tests, exact
statement coverage, documentation, API compatibility, and security checks.
Its implementation is source-equivalent to the released predecessor module,
whose existing race, fuzz, benchmark, mutation, and broker-interoperability
evidence remains applicable; those expensive checks are not repeated solely
because the import path changed. Release acceptance additionally verifies clean
resolution and directly affected consumers.
