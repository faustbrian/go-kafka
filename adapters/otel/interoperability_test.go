...
import (
	...
	"github.com/faustbrian/go-kafka/adapters/otel"
...
func TestTraceContextPropagationAcrossApacheKafka(t *testing.T) {
	...
	topic := fmt.Sprintf("golib-kafkaotel-%d", time.Now().UnixNano())
	createIntegrationTopic(t, ctx, container, topic)

	policy, err := kafkaotel.NewTraceContextPropagation(
	...
	producer, err := kafka.NewProducer(kafka.ProducerConfig{
		...
		ClientID:      "golib-kafkaotel-integration-producer",
	...
	consumer, err := kafka.NewConsumer(kafka.ConsumerConfig{
		Brokers:        []string{broker},
		ClientID:       "golib-kafkaotel-integration-consumer",
		GroupID:        "golib-kafkaotel-integration-v1",
		Topics:         []string{topic},
