# Kafka service integration technical guide
...
`github.com/faustbrian/go-kafka/adapters/service`; consumers must pin a
...
whose existing race, fuzz, benchmark, mutation, and Apache Kafka 4.3.1
interoperability evidence remains applicable; those expensive checks are not
repeated solely because the import path changed. Release acceptance
additionally verifies clean resolution and directly affected consumers. The
predecessor interoperability evidence covers lifecycle ordering, redelivery,
broker loss and recovery, producer ambiguity, commit timeout, and rebalance;
its single-node fixture does not replace the root module's multi-broker,
transaction, or replication evidence.
