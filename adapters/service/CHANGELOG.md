...
- Add `github.com/faustbrian/go-kafka/adapters/service` as the canonical
  target-oriented successor to the released `kafkaservice` module. The
  released path remains supported during the compatibility window.
...
- Establish the implementation that is now published from the canonical
  `github.com/faustbrian/go-kafka/adapters/service` identity.
